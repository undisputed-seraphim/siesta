...
#   ./run_beast.sh --bench            # bench build + load test
#   ./run_beast.sh --profile          # profile build + load test + CPU report
#   ./run_beast.sh --load             # load test only (no build / no profile)
...
run_cpp_tests() {
	...
	local out rc=0
	out=$("$BUILD/echo_beast_client" 2>&1) || rc=$?
	echo "$out"
...
run_python_tests() {
	...
	local out rc=0
	out=$(python3 "$ROOT/echo/test_beast_client.py" 2>&1) || rc=$?
	echo "$out"
...
generate_profile_report() {
	local binary="$BUILD/echo_beast_server_prof"
...
run_integration_tests() {
	...
	local out rc=0
	out=$("$BUILD/echo_beast_integration" 2>&1) || rc=$?
	echo "$out"
	if echo "$out" | grep -q "All tests passed"; then
...
mode_sanity() {
	...
	build_target echo_beast_client
	build_target echo_beast_integration
	...
	local srv_pid
	if ! srv_pid=$(start_server "$BUILD/echo_beast_server") || [[ -z "$srv_pid" ]]; then
...
mode_quick() {
	_export_env
	local failed=0
	"$BUILD/echo_beast_client" 2>&1 || failed=1
	python3 "$ROOT/echo/test_beast_client.py" 2>&1 || failed=1
	if [[ "$failed" -ne 0 ]]; then
...
mode_server() {
	...
	build_target echo_beast_server
	log "starting echo server on ${SERVE}:${PORT}"
	exec "$BUILD/echo_beast_server" "$SERVE" "$PORT"
}
...
mode_bench() {
	...
	: "${CONCURRENCY:=200}"
	ensure_build "bench"
	build_target echo_beast_server_bench

	local srv_pid
	if ! srv_pid=$(start_server "$BUILD/echo_beast_server_bench") || [[ -z "$srv_pid" ]]; then
		fail "could not start server"
		exit 1
	fi
	trap "kill_server $srv_pid" EXIT
...
mode_profile() {
	...
	build_target echo_beast_server_prof
	...
	local srv_pid
	if ! srv_pid=$(start_server "$BUILD/echo_beast_server_prof" \
