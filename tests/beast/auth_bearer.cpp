...
static constexpr uint16_t TEST_PORT = 19980;
static const auto TEST_ADDR = asio::ip::make_address("127.0.0.1");

struct StubServer : Bearer_Auth_API::Server {
	using Bearer_Auth_API::Server::Server;

	void get__echo(const request req, Session::Ptr s) override {
		auto it = req.base().find("Authorization");
		if (it == req.base().end() || std::string(it->value()) != "Bearer correct-token") {
			s->send(s->make_error_response(siesta::Error{
				siesta::ErrorCode::UNAUTHENTICATED, "invalid bearer token"}));
			return;
		}
...
TEST_CASE("bearer token correct auth", "[integration][auth]") {
	asio::io_context ctx;
	auto client = std::make_shared<Bearer_Auth_API::Client>(ctx, "correct-token");
...
TEST_CASE("bearer token wrong auth returns 401", "[integration][auth]") {
	asio::io_context ctx;
	auto client = std::make_shared<Bearer_Auth_API::Client>(ctx, "wrong-token");
	client->start(TEST_ADDR, TEST_PORT);
	ctx.run();
